data "azurerm_key_vault_secret" "storageAccountsAccessKey" {
  name         = var.key_name_storage_account_access_key
  key_vault_id = var.key_vault_id
}

data "azurerm_key_vault_secret" "storageAccountsConnectionString" {
  name         = var.key_name_storage_account_connection_string
  key_vault_id = var.key_vault_id
}
...
resource "azurerm_linux_function_app" "functionApp" {
  ...
  app_settings = merge({
    ...
    DOCKER_REGISTRY_SERVER_USERNAME       = data.azurerm_key_vault_secret.containerRegistryAdminLogin.value
