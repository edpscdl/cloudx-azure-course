...
public class PetStoreServiceImpl implements PetStoreService {
	private final String SEND_PRODUCT_COUNT_METRIC_NAME = "product_sended_count";
	...
	public Collection<Pet> getPets(String category) {
		...
		try {
			...
			log.error("Failed to retrieve pets from PetStorePetService", wce);
			throw new IllegalStateException("Unable to retrieve pets from the PetStorePetService", wce);
		} catch (IllegalArgumentException iae) {
			log.error("Invalid argument when retrieving pets", iae);
	...
	public Collection<Product> getProducts(String category, List<Tag> tags) {
		...
		properties.put("session", this.sessionUser.getSessionId());

		this.sessionUser.getTelemetryClient().trackEvent(
				"[Test][getProducts]", properties, null);
		...
		try {
			...
			this.sessionUser.getTelemetryClient().trackMetric(SEND_PRODUCT_COUNT_METRIC_NAME, products.size());

			if (products.stream().anyMatch(product -> product.getId() > 9)) {
			...
			this.sessionUser.getTelemetryClient().trackException(e, this.sessionUser.getCustomEventProperties(), null);
	...
	public void updateOrder(long productId, int quantity, boolean completeOrder) {
		...
		try {
			...
			log.error("Unable to retrieve order from order service", e);
